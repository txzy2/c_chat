...
{
    User users[] = {
            {.id = 0, .name = "ANTON", .email = "email@email.ru"},
            {.id = 1, .name = "DIMA", .email = "dima@dima.ru"},
    };

    Chat c;
    init_chat(&c, "Диалог 0");
    ...
    ChatMessage msg = {.id = 0, .text = "Test Message", .chat = &c, .from = &users[0], .to = &users[1]};
